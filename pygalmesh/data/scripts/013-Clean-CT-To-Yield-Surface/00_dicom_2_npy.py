...
def auto_crop_to_fit(array, factor):
    """Auto-crop 2D or 3D array to the largest size divisible by factor."""
    if array.ndim == 3:
        z, h, w = array.shape
        new_h = (h // factor) * factor
        new_w = (w // factor) * factor
        return array[:, :new_h, :new_w], (h, w), (new_h, new_w)
    elif array.ndim == 2:
        h, w = array.shape
        new_h = (h // factor) * factor
        new_w = (w // factor) * factor
...
def assert_isotropic_voxels(slice_thickness, pixel_spacing, rel_tol=1e-6):
    """The whole pipeline carries a single scalar voxel edge length.

    01, 03 and 04 all read only SliceThickness and use it for x, y and z
    (see 03: voxel_size = (voxel_dim, voxel_dim, voxel_dim)). PixelSpacing was
    written to the metadata in 010 but never read again, so an anisotropic
    reconstruction would have produced a silently distorted mesh. Check it.
    """
    edges = [slice_thickness, float(pixel_spacing[0]), float(pixel_spacing[1])]
    largest, smallest = max(edges), min(edges)
    if smallest <= 0.0 or (largest - smallest) / largest > rel_tol:
        raise ValueError(
            "Anisotropic voxel grid detected: "
            f"SliceThickness={slice_thickness!r}, PixelSpacing={pixel_spacing!r}. "
            "This pipeline assumes an isotropic reconstruction grid. Either "
            "resample the stack to isotropic voxels or extend 03/04 to carry a "
            "three-component voxel size."
        )
    print(f"✅ Isotropic voxel grid confirmed: {slice_thickness:.9g} mm")


def save_metadata(data, metadata_output_path, num_slices, option, reduce_factor=None, cropping_info=None):
    slice_thickness = float(data.SliceThickness)
    pixel_spacing = [float(ps) for ps in data.PixelSpacing]

    assert_isotropic_voxels(slice_thickness, pixel_spacing)
    ...
    if cropping_info:
        metadata["00_dicom2npy"]["CroppingInfo"] = {
            "OriginalSize": list(cropping_info["original_size"]),
            "CroppedSize": list(cropping_info["cropped_size"])
        }
...
def main():
    ...
    if option == "reduce":
        ...
        for slice_index, instance in enumerate(fs):
            ...
            scan_array, original_size, cropped_size = auto_crop_to_fit(scan_array, reduce_factor)
            ...
            if not metadata_saved:
                ...
                save_metadata(data, metadata_output_path, num_slices, option, reduce_factor, cropping_info)
                metadata_saved = True
