    ...
    for i in range(xy_divisions):
        for j in range(xy_divisions):
            ...
            if np.any(subvol == material_value):
                ...
                # `material_value` in the config is the *array* label 1, i.e. the
                # pore phase. 010 counted those voxels and reported the result as
                # "relative_density", which had density and porosity swapped in
                # every metadata.json. Corrected here; the array itself is
                # untouched.
                total_elements = subvol.size
                pore_count = np.count_nonzero(subvol == material_value)
                solid_count = total_elements - pore_count
                relative_density = solid_count / total_elements
