...
> Aufrufer, `LES_PIPELINE.md` beschreibt die Pipeline von der `.leS`-Datei bis
> zum Netz. `submit_les_pipeline_CLUSTER.sh` reicht Netzvorbereitung und alle
> Punkt-Jobs in einem Aufruf ein.
...
```bash
cd "$HOME/meshing/Meshing/pygalmesh/data/scripts/014-Yield-Surface-From-leS"
...
data/scripts/014-Yield-Surface-From-leS/02_create_folders_CLUSTER.sh
```

This generates the individual jobs and synchronizes the project to:

```text
$HPC_SCRATCH/pygalmesh/data/scripts/014-Yield-Surface-From-leS/
```
...
Without a config argument `config-A01-les.json` is used; see `LES_PIPELINE.md`.
