...
# -> 03 -> 04 -> 05/08/09 -> DolfinX-Netz. Wird von job_prepare_mesh_CLUSTER.sh
# aufgerufen.
set -euo pipefail
...
# Vorverarbeitung: A01_les_2_npy.py liest das segmentierte .leS-Volumen und
# schreibt segmented_3D_volume.npy samt der Metadaten, die 02b erwartet.
...
echo "Using segmented .leS input: $resource_host_path"
...
for subfolder in "$base_subvolume_folder"/*/; do
  ...
  if [ -f "$subfolder/mesh.xdmf" ]; then
    ...
    # 08.09.2026: Nachweis, dass das konvertierte Netz genau EINE flaechen-
    # zusammenhaengende Komponente hat (keine Scharnierteile; CLAUDE.md §22).
    # Exit 1 des Checks bricht die Vorbereitung ab (set -e) - lieber hier als
    # 768 Punkt-Jobs spaeter am Fliessbeginn.
    run_container 1 "" "$SIM_BIND" "$SIM_CONTAINER" python3 "$working_directory/mesh_hinge_check.py" "$subfolder/dlfx_mesh.h5" | tee "$subfolder/dlfx_mesh.hinges.txt"
  fi
done
