...
# MESH_ONLY=1 stops after the mesh + DolfinX conversion and skips the
# elasto-plastic solve. Same preprocessing either way.
...
for subfolder in "$base_subvolume_folder"/subvolume_x*_y*/; do
  ...
  run_python "$SCRIPT_DIR/04_scale_and_translate_mesh.py" --config "$CONFIG_PATH" --mesh "$mesh_output" --center_x "$center_x" --center_y "$center_y"
...
for subfolder in "$base_subvolume_folder"/*/; do
  ...
  if [[ -f "$subfolder/mesh.xdmf" ]]; then
    run_python "$SCRIPT_DIR/make_mesh_dlfx_compatible.py" "$subfolder" -f mesh.xdmf
  fi
done

if [[ "$MESH_ONLY" == "1" ]]; then
  echo "MESH_ONLY=1 -- skipping the elasto-plastic solve."
  echo "Meshes and dlfx_mesh.xdmf are under: $base_subvolume_folder"
  exit 0
fi
