...
# Phase convention (see 01_segment_slice_wise.py):
#   array value 0 = aluminium = the phase that gets meshed
#   array value 1 = pore / air
# 010 ran this audit with material_value = 1, i.e. on the *pore* phase, so
# every "material_*" number in the report actually described pores and vice
# versa. Default corrected to 0 here. The voxel array itself is untouched;
# only what the report calls "material" changes.
DEFAULT_CONFIG = {
    "enabled": False,
    "material_value": 0,
    "pore_value": 1,
...
def main():
    ...
    if cleanup_enabled:
        ...
        # Write both phase labels explicitly. 010 hard-coded the else-branch to 0,
        # which silently produced an all-zero volume as soon as material_value
        # was anything other than 1.
        cleaned = np.where(
            cleaned_mask, cfg["material_value"], cfg.get("pore_value", 1)
        ).astype(volume.dtype, copy=False)
