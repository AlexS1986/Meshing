    ...
    for i in range(xy_divisions):
        for j in range(xy_divisions):
            ...
            if np.any(subvol == material_value):
                ...
                # 👉 Compute relative density (zeros / total elements)
                total_elements = subvol.size
                zero_count = np.count_nonzero(subvol == 0)
                relative_density = zero_count / total_elements

                print(f"💾 Saved: {folder_path}/volume.npy (shape={subvol.shape}, rel_density={relative_density:.4f})")
                ...
                saved_subvolumes.append({
                    ...
                    "relative_density": relative_density  # ✅ Added here
                })
                count += 1
