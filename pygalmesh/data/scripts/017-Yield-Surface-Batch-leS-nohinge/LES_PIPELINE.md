...
`/data/scripts/017-Yield-Surface-Batch-leS-nohinge/A01_segmented/` — dieser Pfad wird
...
```bash
cd "$HPC_SCRATCH/pygalmesh/data/scripts/017-Yield-Surface-Batch-leS-nohinge"
...
## 5a. Mehrere Datensaetze / mehrere Fliessgrenzen (015)

Fuer die Studie aus acht Kombinationen erzeugt man die Configs nicht einzeln,
sondern ueber die Batch-Schicht:

```bash
bash batch_create_configs.sh      # 8 Configs aus config-A01-les.json
bash batch_setup_jobs.sh          # 8 x 96 Punkt-Jobs
```

Die Tabelle der Datensaetze und die Liste der Fliessgrenzen stehen in
`config.sh` (`BATCH_DATASETS`, `BATCH_SIG_Y`). Vollstaendige Beschreibung
inklusive Einreichen und Einsammeln: **`README.md`**.

## 6. Auf dem Cluster laufen lassen

```bash
# 1. Daten und Projekt nach $HPC_SCRATCH synchronisieren, dann:
sbatch "$HPC_SCRATCH/pygalmesh/data/scripts/017-Yield-Surface-Batch-leS-nohinge/job_prepare_mesh_CLUSTER.sh"
...
YIELD_SURFACE_POINTS=192 data/scripts/017-Yield-Surface-Batch-leS-nohinge/02_create_folders_CLUSTER.sh

# danach die komplette Kette einreihen
"$HPC_SCRATCH/pygalmesh/data/scripts/017-Yield-Surface-Batch-leS-nohinge/submit_les_pipeline_CLUSTER.sh"
```
