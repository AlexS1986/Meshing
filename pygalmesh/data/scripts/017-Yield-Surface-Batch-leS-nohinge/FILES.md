...
| Ordner | Inhalt |
|---|---|
| `mesh_hinge_check.py` | Diagnose (08.09.2026): flächen- vs. knotenbasierte Zusammenhangskomponenten eines Tet-Netzes; findet „Scharnierteile“ (nur über Kanten/Knoten angebunden, Starrkörpermoden, Ursache der Newton-Nichtkonvergenz am Fließbeginn, `CLAUDE.md` §22). Im Container ausführen; Exit 1, wenn mehr als eine Flächenkomponente. Der Konverter `make_mesh_dlfx_compatible_cluster.py` entfernt solche Teile seit 08.09.2026 (`--keep-hinges` stellt das alte Verhalten her, `--hinge-min-anchor`). |
