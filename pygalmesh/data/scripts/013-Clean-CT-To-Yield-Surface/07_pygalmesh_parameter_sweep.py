...
def main():
    ...
    for variant in variants:
        ...
        if args.skip_existing and tetgen_log_path.exists():
            ...
            # 013 has only the SDF/pygalmesh path, so the sweep writes into
            # sdf_pygalmesh_parameters.pygalmesh_parameters instead of the
            # removed top-level pygalmesh_parameters block.
            mesh_cfg = config["03_mesh_3D_array"]
            mesh_cfg["mesh_output_path"] = str(mesh_path)
            sdf_params = mesh_cfg.setdefault("sdf_pygalmesh_parameters", {})
            pygalmesh_params = sdf_params.setdefault("pygalmesh_parameters", {})
            pygalmesh_params["verbose"] = False
            pygalmesh_params.update(variant.get("pygalmesh_parameters", {}))
            if "sdf_pygalmesh_parameters" in variant:
                sdf_params.update(variant["sdf_pygalmesh_parameters"])
