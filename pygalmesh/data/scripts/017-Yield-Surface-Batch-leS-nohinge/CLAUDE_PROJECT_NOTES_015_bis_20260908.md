...
**08.09. 12:30 — URSACHE DER NICHTKONVERGENZ GEFUNDEN: „Scharnierteile" im
Netz (Zellgruppen nur ueber Kanten/Knoten angebunden).** Neue Pruefung
`mesh_hinge_check.py` (flaechenbasierte Zusammenhangskomponenten: zwei Tets
sind nur Nachbarn, wenn sie eine Dreiecksflaeche teilen) auf den vier r4-
Netzen: knotenbasiert je 1 Komponente (Fragmentfilter 05.09. hat funktioniert),
flaechenbasiert aber JM-25-77: 88 Komponenten (1188 Tets = 0,038 % in 87
Teilen, groesste 491/273/254 Tets), JM-25-88: 90 (130 Tets), JM-25-71: 141
(612 Tets), JM-25-83: 254 (999 Tets); ueberwiegend Einzel-Tets und Paare.
Mechanik: ein Tet an einer Kante hat eine, an einem Knoten drei Starrkoerper-
rotationen -> K in exakter Arithmetik singulaer; anders als bei den echten
Inseln (Pivot exakt 0 -> MUMPS -10) ist das Pivot hier rundungsklein, MUMPS
faktorisiert "erfolgreich", die Loesung in diesen Freiheitsgraden ist Unsinn.
**Beweis am Snapshot** (`scratch/hinge_alpha_check.py`, Flaechenkomponenten auf
der Snapshot-Topologie): JM-25-71 ys_000 (t = 0,00225): |u| Hauptteil max
9,8e-3 mm, **Scharnierknoten max 1,99e+11 mm**; JM-25-77 ys_082 (t = 0,0265):
Hauptteil max 0,083 mm, Scharnier p99 0,43 / max 0,48 mm. Solche Komponenten
setzen dem Residuum einen Rundungsboden (eps * |K u| mit u ~ 1e11), den
Newton nicht unterschreiten kann -> "maximum number of iterations" unabhaengig
von dt (beobachtet bis dt = 7,8e-7), in r2 wie r4, unabhaengig von H und
max_it. Erwartungsgemaess ist JM-25-71 (explodiertes Teil) am schlimmsten,
JM-25-77 (max 0,48 mm) am gutmuetigsten.
Korrektur meiner Hypothese: die Plastizitaet sitzt NICHT in den Scharnier-
teilen (alpha-Integral dort 0,0-0,2 %; Top-20-Zellen nach alpha alle im
Hauptteil, alpha 0,63-1,04 bei ys_082 bzw. 0,08-0,13 bei ys_000 = Ein-Element-
Stegquerschnitte/Slivers, physikalisch plausibel), sondern die Scharniere
verderben die Iteration. Der alpha-Wert 1,04 in Einzelzellen ist fuer die
Kriterien unerheblich (20 Zellen / 3 Mio.), sollte aber im Paper als
Netzeffekt erwaehnt werden (Kleindehnung, Ein-Element-Stege).
**Fix:** `make_mesh_dlfx_compatible_cluster.py` (Backup
`.vor_scharnierfilter_20260908`): neue Funktionen `face_components` und
`drop_hinge_components`, laufen nach dem Fragmentfilter; behalten wird die
groesste Flaechenkomponente plus Teile mit >= `--hinge-min-anchor` (Default 4)
Knoten in der Randschale (dort fixiert Dirichlet alle Freiheitsgrade);
`--keep-hinges` = altes Verhalten; Report in `dlfx_mesh.components.txt`
(angehaengt). Synthetisch getestet (Wuerfelnetz + Kanten-Tet + Knoten-Tet +
Flaechen-Tet + verankertes 2er-Teil): Kanten-/Knoten-Tet entfernt, Flaechen-Tet
und verankertes Teil behalten, Neunummerierung konsistent.
**Konsequenz:** anderes Netz -> alle Snapshots unbrauchbar (Restart prueft das
und bricht ab) -> Studie 015 r4 komplett frisch rechnen (768 Punkte, mit H = 70
und alpha-Kriterium; Kostenschaetzung 2-4 h je Punkt ohne Kriechen). Alte
Ergebnisse (107 Punkte mit alpha >= 1e-3, alle mit Scharnieren gerechnet)
archivieren, nicht mischen. Laufende (25) und gehaltene (586) Jobs sowie die
NEWTON_LOG-Diagnosejobs abbrechen. Restart-Fix von heute frueh bleibt wertvoll
(Walltime-Fortsetzung).

**Entscheidung Nutzer 08.09. ~13:00: alles neu rechnen, in einem NEUEN
Studienordner, damit 015 (r2- und r4-Ergebnisse, Snapshots, Logs) unangetastet
als Archiv bleibt.** Begruendung fuer den neuen Ordner statt eines neuen Tags in
015: run_id/binning_label haengen nur an LES_REDUCE_FACTOR (`batch_lib.sh`
Z. 49-52), ein neuer r4-Lauf in 015 wuerde mit `_les_r4`-Ordnern kollidieren;
ein Tag-Mechanismus waere ein Eingriff in die Batch-Logik. Ordnerkopie mit
Namensersetzung ist das etablierte Verfahren (014 -> 015). Neuer Ordner:
`017-Yield-Surface-Batch-leS-nohinge` (016 = Fracture-Studie). Kopiert werden
Skripte, Configs, 00_template, Doku; NICHT yield_surface_jobs/runs, 00_results,
*_segmented, A01_segmented, Backups, _to_delete, __pycache__. Netze werden dort
komplett neu vorbereitet (A01 -> 03 -> Konverter mit Scharnierfilter); in
`run_prepare_mesh_CLUSTER.sh` laeuft `mesh_hinge_check.py` jetzt direkt nach
dem Konverter und bricht die Vorbereitung bei > 1 Flaechenkomponente ab
(Report `dlfx_mesh.hinges.txt`). 015 bleibt eingefroren; Snapshots dort
(~1 TB) koennen spaeter auf restart_meta/graphs/logs eingedampft werden.
